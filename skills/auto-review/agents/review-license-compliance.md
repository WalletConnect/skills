...
> **Local mode:** Review the current branch's changes (git diff against the base branch) and the local working tree — not a GitHub PR. Use `git diff`, `Read`, `Grep`, `Glob` to inspect the dependency manifest changes and full file context. There is no PR to comment on; return your findings as text to the caller.
...
1. Read the diff to identify **newly added dependencies** (ignore version bumps of existing deps)
