#!/usr/bin/env node
/**
 * Security Queue Report — Generate & Deliver via Anthropic Messages API
 *
 ...
 *   HUBSPOT_API_KEY    - Required for the Python report script (HubSpot API)
...
const scriptPath = join(resolvedSkillDir, "scripts", "security_queue_report.py");
...
const slackPrompt = [
  "You are a security operations reporting assistant. Below is a HubSpot security pipeline queue report.",
  "Compose a concise Slack summary using Slack mrkdwn formatting.",
  "",
  "Guidelines:",
  "- Lead with total ticket count and unassigned count (use emoji: 🔒 security, ⚠️ unassigned, 🤖 AI triage)",
  "- Highlight tickets needing immediate attention (unassigned, high priority)",
  "- Show breakdown by pipeline stage",
  "- Mention AI triage coverage (triaged vs pending)",
  "- Keep it under 2000 characters",
  "- If there are zero tickets, say the queue is clear",
